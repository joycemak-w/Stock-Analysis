...
def rsi_macd_chart(selected_symbol):
    ...
    ax1.set_title(f'RSI and MACD of {symbols[selected_symbol]}')
    ...
    # Second chart
    # Plot the RSI
    ax2.set_title(f'Relative Strength Index of {symbols[selected_symbol]}')
    ...
    # Third chart
    # Plot the MACD
    ax3.set_title(f'Moving Average Convergence/Divergence of {symbols[selected_symbol]}')
    ...
    ax3.scatter(df.index[df['Golden_Cross']], df['MACD'][df['Golden_Cross']], marker='^', color='g', label='Golden Cross')
    ax3.scatter(df.index[df['Death_Cross']], df['MACD'][df['Death_Cross']], marker='v', color='r', label='Death Cross')
    ax3.legend()
    # Display the charts
    plt.show()
    # return fig
...
def rsi_macd_combine_chart(selected_symbol):
    query = "SELECT [date],[close] FROM Stocks WHERE symbol = '%s'" % selected_symbol 
    df = pd.read_sql(query, conn)
    # df.to_csv('./rsi_macd.csv', index=0)
    # df = pd.read_csv("./rsi_macd.csv") 
...
# rsi_macd_combine_chart('2888.HK')
